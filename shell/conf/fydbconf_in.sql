...
CREATE TABLE `tfilecopyinfo` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=881983 DEFAULT CHARSET=gbk;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `tsysconfig` VALUES ('fychmanager','port','6001','调度主进程监听端口'),('fyftpmanager','port','6002','FTP业务管理主进程监听端口'),('fyflowcontrol','port','6003','流控进程监听端口'),('ferryway','ip','127.0.0.1','本机监听IP'),('fiber','ip','10.10.13.223','光纤目标端IP'),('device','device_flag','3','1-外端机，2-内端机'),('device','device_name','receiver','外端机'),('flowcontrol','flowlimit','100','流控大小'),('flowcontrol','unit','2','流控单位1-Kb，2-Mb，3-Gb'),('receiver','ip','192.168.0.189','服务发送端IP'),('receiver','port','6001','服务发送端port'),('thread','number','1','工作线程数'),('fystreamanager','port','6004','stream业务管理主进程监听端口'),('fyftpserver','port','21','ftpserver监听端口'),('managerip','ip','192.168.50.100','管理口IP'),('businessip','ip','192.168.50.100','业务口IP'),('channelip','ip','127.0.0.1','通道IP'),('fycustproto','port','6005','自定义协议通道管理主进程监听端口'),('islongorshortconnect','flag','2','1-长连接；2-短连接'),('ferryway','ethname','eth7','单向网卡名称'),('ferryway','maxtasknum','15','最大通道数'),('ferryway','maxtime','120','超时时间，单位秒'),('fyflowcontrol','toport','6000','数据交换端口'),('ferryway','runmode','1','1-单向；2-双向'),('fysmbmanager','port','6006','smb进程监听端口'),('fynfsmanager','port','6007','nfs进程监听端口'),('ftpconnect','number','1000','ftp连接数'),('ftpclientsendtimeout','timeout','120','ftpclient发送数据超时（秒）'),('fyflowcontrol','sendtoport','6000','数据交换端口'),('datarestore','flag','0','1-yes;0-no'),('needsyncing','flag','0','1-yes;0-no'),('hoststand','ishostflag','0','1-on;0-off'),('hoststand','masterflag','1','1-master;2-slave'),('hoststand','ip','127.0.0.1','ip'),('hoststand','port','6008','port'),('filecheck','flag','0','1-on;0-off'),('diskLog','setPercent','90','日志磁盘使用告警阈值(%)'),('fysftpmanager','port','6010','fysftpmanager port'),('writetolocal','flag','1','1-yes;0-no');
...
-- Dump completed on 2016-11-16 11:40:42
